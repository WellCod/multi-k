...
function BarraH({ label, value, max, extra }: {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-28 text-xs text-gray-600 text-right capitalize truncate shrink-0">
        {label}
      </span>
      ...
      <span className="text-xs text-gray-700 w-20 text-right shrink-0">
        {value}{extra ? ` ${extra}` : ""}
      </span>
    </div>
  );
...
function TabelaProducao({ dados }: { dados: ProducaoOut[] }) {
  if (dados.length === 0) {
    return <p className="text-sm text-gray-500">Sem propostas no período.</p>;
  }
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {dados.map((r) => (
            <tr key={r.corretor_id} className="border-b hover:bg-gray-50">
              <td className="px-3 py-2 font-medium">{r.corretor_nome}</td>
              <td className="px-3 py-2 text-right">{r.cotacoes}</td>
              <td className="px-3 py-2 text-right">{r.propostas}</td>
              <td className="px-3 py-2 text-right">{fmtPct(r.taxa_conversao)}</td>
              <td className="px-3 py-2 text-right font-mono">{fmtReal(r.premio_total)}</td>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gray-50 border rounded p-3 text-center">
          <p className="text-2xl font-semibold">{dados.total_cotacoes}</p>
          <p className="text-xs text-gray-500 mt-0.5">Cotações</p>
        </div>
        <div className="bg-gray-50 border rounded p-3 text-center">
          <p className="text-2xl font-semibold">{dados.total_com_proposta}</p>
          <p className="text-xs text-gray-500 mt-0.5">Com proposta</p>
        </div>
        <div className="bg-gray-50 border rounded p-3 text-center">
          <p className="text-2xl font-semibold">{fmtPct(dados.taxa_conversao_geral)}</p>
          <p className="text-xs text-gray-500 mt-0.5">Conversão geral</p>
        </div>
      </div>
      {dados.por_ramo.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Por ramo</p>
          {dados.por_ramo
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-gray-700">Produção por corretor</h2>
          ...
          {/* Funil */}
          <div className="bg-white border rounded-lg p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Funil de conversão</h2>
            {funil && <Funil dados={funil} />}
          </div>

          {/* Mix */}
          <div className="bg-white border rounded-lg p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">Mix por ramo</h2>
            {mix && <Mix dados={mix} />}
          </div>
        </div>
      )}
