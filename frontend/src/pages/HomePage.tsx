          ...
          <tbody>
            {items.map((r) => (
              <tr key={r.proposta_id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-3 py-2 capitalize">{r.ramo}</td>
                <td className="px-3 py-2">{fmtReal(r.premio_total)}</td>
                <td className="px-3 py-2">{fmtDate(r.fim_vigencia)}</td>
    ...
    <section>
      ...
      <div className="space-y-2">
        {items.map((p) => (
          <div
            key={p.cotacao_id}
            className="flex items-center justify-between bg-white border rounded px-4 py-2.5 text-sm"
          >
            <div>
              <span className="capitalize font-medium">{p.ramo}</span>
              <span className="text-gray-400 text-xs ml-2">{fmtDatetime(p.criado_em)}</span>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-gray-700">{fmtReal(p.premio_total)}</span>
              <button
    ...
    <section>
      ...
      <div className="space-y-2">
        {items.map((c) => (
          ...
          >
            <div>
              <span className="capitalize font-medium">{c.ramo}</span>
              <span className="text-gray-400 text-xs ml-2">{fmtDatetime(c.criado_em)}</span>
            </div>
            <button
              className="text-xs text-indigo-600 hover:underline"
    ...
    <section>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {items.map((p) => (
              <tr key={`${p.proposta_id}-${p.numero_parcela}`} className="border-b">
                <td className="px-3 py-2 font-mono text-xs">{p.protocolo}</td>
                <td className="px-3 py-2">{p.numero_parcela}ª</td>
                <td className="px-3 py-2">{fmtDate(p.vencimento)}</td>
                <td className="px-3 py-2">{fmtReal(p.valor)}</td>
                <td className="px-3 py-2 text-green-700">{fmtReal(p.comissao)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold">Minha fila</h1>
        <button
    ...
    <div className="space-y-2">
      {items.map((item) => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="w-24 text-xs text-gray-600 text-right capitalize shrink-0">
            {item.label}
          </span>
          ...
          <span className="text-xs text-gray-700 w-8 text-right shrink-0">{item.value}</span>
        </div>
      ))}
    </div>
  );
