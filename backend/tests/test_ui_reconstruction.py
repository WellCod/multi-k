...
def test_every_carrier_survives_partial_and_failed_results(states: list[str]) -> None:
    jobs = [
        CotacaoJob(
            cia=f"test-{i}",
            status="processando" if state == "processando" else "concluido",
            status_resultado=None if state == "processando" else state,
            criado_em=datetime.now(UTC),
            premio_total=Decimal("1234.56") if state == "sucesso" else None,
            necessita_vistoria=False,
            restricoes=[],
            mensagens=[],
            payload_resposta=None,
        )
...
async def test_draft_roundtrip_encryption_and_conflict(
    ...
    body = {
        ...
        "dados": {
            "ramo": "auto",
            "step": 2,
            "step1": {"nome": "Teste exclusivo do rascunho"},
        },
    }
    ...
    stored = (
        await db.execute(
            text("SELECT dados FROM rascunhos_cotacao WHERE usuario_id = :id"),
            {"id": user.id},
        )
    ).scalar_one()
...
async def test_cancelled_job_never_calls_adapter(
    ...
    q = select(CotacaoJob).where(CotacaoJob.cotacao_id == quote_id)
