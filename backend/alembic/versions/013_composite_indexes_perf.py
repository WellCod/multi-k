...
def upgrade() -> None:
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_propostas_usuario_transmitida "
        "ON propostas (usuario_id, transmitida_em DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_cotacoes_usuario_criado "
        "ON cotacoes (usuario_id, criado_em DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_cotacao_jobs_cot_cia_status "
