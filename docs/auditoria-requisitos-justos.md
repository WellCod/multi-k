...
| insured separado de main_driver (J2 §4) | `adapter.py::_payload_cotacao` | Parcial: estrutura v2 existe, falta testar variantes e validação dos campos obrigatórios. |
...
| Placa/chassi, FIPE, ano, bônus e leilão (J2 §4) | Adapter + formulário Auto (`leilao`) | Parcial: `is_auction` passou a ser coletado e encaminhado neste lote; validações cruzadas entre placa, chassi, FIPE e ano continuam sem teste. |
| Comissão inteira 10–25 na cotação (J1/J2 §4) | `_comissao_cotada` no adapter; `comissao_pct_cotada` no payload; `prepare_transmission`; teto de 30% no servidor | Corrigido neste lote: a cotação usa a comissão configurada por CIA/ramo, fora da faixa 10–25 é erro explícito, e a transmissão só registra a comissão cotada — divergência exige recotação. Falta validar a faixa por CIA já no cadastro de comissão. |
...
| info opcional, texto separado, detalhe/print (J4) | Adapter guarda info mas copia para mensagens; PDF comparativo não o inclui | Parcial: separar campo informativo, limitar conforme contrato e testar detalhe/PDF. Não extrair preços do texto. |
| Formalização: email, telefone, tipo e installments anual (J2 §7) | `client.py::converter_proposta`, adapter e modal | Parcial: chamadas existem; falta validar coerência da opção confirmada no servidor e campos de contato antes da chamada externa. |
...
| Valores em reais/Decimal (J3 §6 e regra interna) | Novo payment usa Decimal; adapter original ainda passa por float/defaultzero | Parcial: eliminar conversões e zeros artificiais restantes com testes negativos. |
