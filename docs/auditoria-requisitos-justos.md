...
| Comissão na cotação (J1/J2 §4 + resposta de 17/09) | `_comissao_cotada`, `_comissao_confirmada`, `prepare_transmission`, teto no cadastro | Corrigido: a comissão gravada passa a ser **a devolvida pela seguradora**, não a enviada — em renovação de apólice da Justos um piso é aplicado na criação da cotação e o percentual volta diferente, sem erro. A faixa é declarada em `Capacidades` e validada no cadastro. Faixa real confirmada: **0 a 25**, não 10 a 25 como diz a documentação. |
...
| ci_code obrigatório quando há bônus (J2 §7 + resposta de 17/09) | `_bonus_anterior` e guarda em `transmitir`; campo condicionado ao bônus no formulário | Corrigido: a seguradora confirmou que **o gatilho é a classe de bônus, não a renovação**. Bônus maior que zero sem CI não transmite, inclusive em negócio novo com bônus transferido. A regra anterior errava nos dois sentidos. |
...
| Exportação updatedSince, skip/take≤100, policyId (J3 §4) | `client.py::exportar_apolices`, `adapter.py::movimentos` | Parcial: paginação existe; sem consumidor nem agendador. Perde precisão para date e falha HTTP encerra o laço como parcial. Desenho e etapas fechados no plano; cadência confirmada em **uma varredura por hora**, e a carteira em produção tem 54 apólices. |
| ACTIVE/INACTIVE não determina causa de encerramento (J3 §4.3) | `movimentos` converte INACTIVE em cancelamento, demais em emissão | Divergência mantida. A seguradora confirmou que **não existe campo com a causa** e perguntou se é bloqueante para nós — resposta pendente do responsável. Sem o campo, o evento deve dizer apenas que a apólice encerrou. |
| Incremental, deduplicação e vínculos de renovação (J3 §4–5) | Retorna policyId, descarta previousPolicyId/parte dos dados | Lacuna: cursor durável por updatedAt e projeções idempotentes seguem não implementados. Desenho e etapas em `justos/plano-exportacao-incremental.md`, já revisados com as respostas de 17/09. `previousPolicyId` cobre só renovação dentro da Justos, sem previsão de mudar. Gate de fase antes de ativar sincronização. |
...
Respostas da seguradora (17/09/2026): as sete perguntas foram respondidas e
duas invalidaram código já em uso — a obrigatoriedade do `ci_code`, que segue o
bônus e não a renovação, e a comissão gravada, que passa a ser a devolvida na
resposta. Ambas corrigidas com teste. A API de exportação segue **em
homologação**, com layout sujeito a ajuste.
